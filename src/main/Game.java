...
public class Game extends JFrame
	...
	private void setGame() {
		...
		inventory.add(new Item(Constants.TYPE_TOOL, 0, 1));
		inventory.add(new Item(Constants.TYPE_TOOL, 1, 1));
		inventory.add(new Item(Constants.TYPE_TOOL, 2, 1));
		inventoryFocus = 0;
		running = true;
		viewX = 0;
		viewY = 0;
...
>>>>>>> 08fd8053c80e83e47f5a32625390e4ccbd52731c
	...
	private void gameLoop() {
		...
		while (running) {
			...
			if (!paused) {
				// Do as many game updates as we need to, potentially playing catchup.
				while (now - lastUpdateTime > Constants.TIME_BETWEEN_UPDATES
				...
				// get rid of this.
				if (now - lastUpdateTime > Constants.TIME_BETWEEN_UPDATES) {
...
=======
				// Render. To do so, we need to calculate interpolation for a smooth render.
				float interpolation = Math.min(1.0f, (float) ((now - lastUpdateTime) / Constants.TIME_BETWEEN_UPDATES));
				drawGame(interpolation);
				lastRenderTime = now;
				...
				while (now - lastRenderTime < Constants.TARGET_TIME_BETWEEN_RENDERS
						&& now - lastUpdateTime < Constants.TIME_BETWEEN_UPDATES) {
					Thread.yield();
...
>>>>>>> 08fd8053c80e83e47f5a32625390e4ccbd52731c
	...
	private void updateGame() {
		...
		if (updateIntervalCount >= updateInterval) {
			updateIntervalCount = 0;
			world.update((int) Math.floor(viewX / wBlockSize), (int) Math.floor((viewX + viewW) / wBlockSize),
					(int) Math.floor(viewY / wBlockSize), (int) Math.floor((viewY + viewH) / wBlockSize));
		}
		updateIntervalCount++;
		if (player.getJumpSequence() > 2) {
			addWorldObject(new Particle(player.getX() + 8, player.getY() + 8, 0, 0, 30));
...
>>>>>>> 4f7bfb7e869ece17d898350ba3b3462c23fd44e3
		...
		public void paintComponent(final Graphics g) {
			...
			// Draw Terrain
			for (int i = (int) Math.floor(viewX / wBlockSize); i < Math
					.floor((viewX + viewW + 128) / wBlockSize); i++) {
				for (int j = (int) Math.floor(viewY / wBlockSize); j < Math
						.floor((viewY + viewH + 128) / wBlockSize); j++) {
					...
					if (wGridBounds(i, j) && world.getBID(i, j) != 0 && world.getWID(i, j) == 0
						...
						g.drawImage(sprites[Constants.TYPE_BACK][world.getBID(i, j)],
								(int) (i * wBlockSize - viewXFinal), (int) (j * wBlockSize - viewYFinal), wBlockSize,
								wBlockSize, null);
					}
					if (wGridBounds(i, j) && world.getWID(i, j) != 0) {
			...
			for (int i = 0; i < objList.size(); i++) {
				...
				if (xx + vx < vx + vw && xx + ww + vx > vx && yy + vy < vy + vh && yy + hh + vy > vy) {
					...
					if (obj instanceof Item_Drop) {
						BufferedImage image = null;
						image = sprites[((Item_Drop) obj).getType()][((Item_Drop) obj).getId()];
						g.drawImage(image, (int) (obj.getX() - viewXFinal), (int) (obj.getY() - viewYFinal),
			...
			if (world.getWID(Math.floorDiv((int) (mouseX + viewXFinal), wBlockSize),
					Math.floorDiv((int) (mouseY + viewYFinal), wBlockSize)) != 0) {
				g.setColor(Color.white);
				g.drawRect(xx, yy, wBlockSize, wBlockSize);
			}
			g.drawImage(spr_black, (int) mouseX, (int) mouseY, 16, 16, null);
	...
	public void pressMouse(double pX, double pY) {
		if (Math.abs(pX + viewXFinal - player.getX()) < 128
			...
			int yy = Math.floorDiv((int) ((int) pY - 24 + viewYFinal), wBlockSize);
			int xxx = xx * wBlockSize + wBlockSize / 4;
			int yyy = yy * wBlockSize + wBlockSize / 4;
			if (inventory.size() > 0 && inventory.get(inventoryFocus).getCount() > 0
				...
				// Place Block
				if (inventory.get(inventoryFocus).getType() == Constants.TYPE_BLOCK) {
				...
				// Place Back
				if (inventory.get(inventoryFocus).getType() == Constants.TYPE_BACK) {
				...
				if (inventory.get(inventoryFocus).getType() == Constants.TYPE_TOOL
						&& inventory.get(inventoryFocus).getId() == Constants.PICKAXE) {
				...
				if (inventory.get(inventoryFocus).getType() == Constants.TYPE_TOOL
						&& inventory.get(inventoryFocus).getId() == Constants.AXE) {
					Player.setSwinging(true);
					if (world.getBID(xx, yy) != Constants.BACK_AIR) {
...
=======
				if (inventory.get(inventoryFocus).getType() == Constants.TYPE_TOOL
					...
					Player.setSwinging(true);
					double x = pX + viewXFinal;
					double y = pY + viewYFinal;
					for (int i = 0; i < objList.size(); i++) {
						WorldObject obj = objList.get(i);
						if (obj instanceof Animal) {
				...
				// Eat Food
				if (inventory.get(inventoryFocus).getType() == Constants.TYPE_FOOD) {
					player.setHunger(player.getHungerMax());
					inventory.get(inventoryFocus).changeCount(-1);
				}
			}
